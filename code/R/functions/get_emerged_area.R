# THESE FUNCTIONS ARE SUPPOSED TO BE USED WITH THE 03_emerged_area_pct.R and 04_quantitative_pct_calculation.R scripts.
...
batch_reclass_and_save <- function(raslist, raster_path, rclmat, outpath){
  ...
  for (i in 1:length(raslist)) {
    ...
    writeRaster(rc_clamped, filename = paste0(outpath, "r_", 
...
get_zonal_histo <- function(polys, raster_files, raster_path, out_path) {
  ...
  for (i in 1:length(raster_files)) {
    ras_files <- list.files(raster_path, pattern = "*tif$", 
...
get_emersion_data <- function(a, column_index) {
  ...
  mutate(across(where(is.integer), as.numeric)) # Rowsums needs numeric fields
  ...
  aa <- aa %>%
    ...
    mutate(AreaLt25 = ifelse(AlwaysEm == 0 & AlwaysSub == 0 & Area100 == 0 &
